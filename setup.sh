...
VERSION=$(gcloud app instances list -s default --format 'value(version)')
...
#10. Setup superuser WIP/TODO
SU_PASSWORD=$(echo $RANDOM | base64 | head -c 20;)
#gcloud app instances ssh --service=default --version=$VERSION $INSTANCEID --container=gaeapp -- python manage.py shell -c "from django.contrib.auth.models import User; User.objects.create_superuser('admin', 'admin@example.com', '$(echo $SU_PASSWORD)')"
#gcloud app instances ssh --service=default --version=$VERSION $INSTANCEID --container=gaeapp -- DJANGO_SUPERUSER_USERNAME=admin DJANGO_SUPERUSER_PASSWORD=$SU_PASSWORD DJANGO_SUPERUSER_EMAIL="admin@admin.com" python manage.py createsuperuser --noinput

echo "Generated Default Password: $SU_PASSWORD
"

echo "Please SSH into the instance and create the superuser manually
"
open https://$PROJECT_ID.uc.r.appspot.com/admin

# #CLEANUP/ Revert file changes
